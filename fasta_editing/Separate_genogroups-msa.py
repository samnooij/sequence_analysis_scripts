...
aa_file = seq_directory + "VP1-manual-translatorx.aa_ali.fasta"
nt_file = seq_directory + "VP1-manual-translatorx.nt_ali.fasta"
...
for group in genogroup_dict_aa.keys():
    filename = "%sseparate-aa/VP1_aa-MSA-%s.fasta" % (seq_directory, group)
...
for group in genogroup_dict_nt:
    filename = "%sseparate-nt/VP1_nt-MSA-%s.fasta" % (seq_directory, group)
